...
RUN git clone https://github.com/andrewrk/groovebasin.git /home/groovebasin-source

RUN cp /home/groovebasin-source/package.json .
RUN npm install

RUN cp -a /home/groovebasin-source/. .
RUN npm run build

# Fix Youtube import error by updating ytdl-core module
RUN npm install ytdl-core@~0.28.3 --save
